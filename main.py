...
def process_buy(message):
    try:
        ...
        if not product:
            ...
            return
        bot.send_message(message.chat.id, "Send Quantity")
...
def deposit_amount(message):
    try:
        ...
        bot.send_message(
            message.chat.id,
            f"Send Payment Then Send TRXID\n\nbKash: 01XXXXXXXXX\nNagad: 01XXXXXXXXX\nAmount: {amount}"
        )
...
def save_trx(message, amount):
    trxid = message.text
    cursor.execute("INSERT INTO deposits (user_id,amount,trxid) VALUES(?,?,?)", (message.from_user.id, amount, trxid))
    conn.commit()
    bot.send_message(message.chat.id, "✅ Deposit Submitted")
...
def add_product(message):
    if message.from_user.id not in ADMINS:
        return
    bot.send_message(message.chat.id, "Name:Price")
...
def add_stock(message):
    if message.from_user.id not in ADMINS:
        return
    bot.send_message(message.chat.id, "ProductID:Stock")
...
def stock(message):
    if message.from_user.id not in ADMINS:
        return
    cursor.execute("""
        SELECT products.name, COUNT(stock.id)
        FROM stock
    ...
    rows = cursor.fetchall()
    if not rows:
        bot.send_message(message.chat.id, "No Stock")
        return
    text = "Stock Status\n\n"
    for r in rows:
        text += f"{r[0]} : {r[1]}\n"
...
def deposits(message):
    if message.from_user.id not in ADMINS:
        return
    cursor.execute("SELECT id,user_id,amount,trxid FROM deposits WHERE status='pending'")
    rows = cursor.fetchall()
    if not rows:
...
def approve(message):
    if message.from_user.id not in ADMINS:
        return
    try:
        deposit_id = int(message.text.split()[1])
        cursor.execute("SELECT user_id,amount FROM deposits WHERE id=?", (deposit_id,))
        row = cursor.fetchone()
        if not row:
            bot.send_message(message.chat.id, "Deposit Not Found")
            return
        user_id, amount = row
        ...
        conn.commit()
        bot.send_message(message.chat.id, "✅ Deposit Approved")
        bot.send_message(user_id, f"✅ {amount} TK Added To Balance")
    except:
        bot.send_message(message.chat.id, "Usage: /approve deposit_id")
